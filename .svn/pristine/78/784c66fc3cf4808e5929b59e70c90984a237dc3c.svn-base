...
public class PatientActivity extends Activity {
	...
	protected void onResume() {
		...
		if (this.patient.hasVisited()) {
			this.visitTimes = this.patient.getVisitTimes();
			// Reverse the list
			ArrayList<TimeStamp> reversed = new ArrayList<TimeStamp>();
			for (int i = this.visitTimes.size() - 1 ; i >= 0; i--){
				reversed.add(this.visitTimes.get(i));
			}
			this.visitTimes = reversed;
		} else {
